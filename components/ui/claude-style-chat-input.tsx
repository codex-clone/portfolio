...
import { Plus, ChevronDown, ArrowUp, X, FileText, Loader2, Check, Archive, MessageSquare } from "lucide-react";
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            <button
                ...
                className={`inline-flex items-center justify-center relative shrink-0 transition font-base duration-300 ease-out h-8 rounded-xl px-3 min-w-[4rem] active:scale-[0.98] whitespace-nowrap !text-xs pl-2.5 pr-2 gap-1 
                ${isOpen
...
export const ClaudeChatInput: React.FC<ClaudeChatInputProps> = ({ onSendMessage }) => {
    ...
    return (
        ...
        >
                            ...
                            <div>
                                <button
                                    ...
                                    className={`
                                        ...
                                        ${hasContent
                                            ? 'bg-accent-main text-white hover:bg-accent-hover shadow-md'
                                            : 'bg-accent-main/30 text-white/60 cursor-default'}
                                    `}
                                    type="button"
            ...
            {
                isDragging && (
                    <div className="absolute inset-0 bg-bg-200/90 border-2 border-dashed border-accent-main rounded-2xl z-50 flex flex-col items-center justify-center backdrop-blur-sm pointer-events-none">
                        <Icons.Archive className="w-10 h-10 text-accent-main mb-2 animate-bounce" />
                        <p className="text-accent-main font-medium">Drop files to upload</p>
                    </div>
                )
            }
            ...
            <input
                ...
                type="file"
                multiple
                className="hidden" // Ensure this is hidden
                onChange={(e) => {
